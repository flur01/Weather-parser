...
def login():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        user = db.session.query(User).filter(User.name == form.name.data and User.email == form.email.data).first()
...
def add_user():
    ...
    if request.method == 'POST' and form.validate_on_submit() :
        ...
        new_remember = form.remember.data
...
def feedback():
    form = AddFeedback()
    if request.method == 'POST' and form.validate_on_submit():
        user = db.session.query(User).filter(User.email == form.email.data and User.name == form.email.name ).first()
